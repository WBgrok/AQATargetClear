...
def Main():
    ...
    NumbersAllowed = []
    Targets = []
    MaxNumberOfTargets = 20 # length of the train of target
    MaxTarget = 0 # Max value for a target number
    MaxNumber = 0 # Mac value for an available (player) number
    TrainingGame = False # The training game is here so they can use test cases without randomness
...
def PlayGame(Targets, NumbersAllowed, TrainingGame, MaxTarget, MaxNumber):
    ...
    while not GameOver:
        ...
        print()
        if CheckIfUserInputValid(UserInput): # first checck 
            UserInputInRPN = ConvertToRPN(UserInput) # convert to RPN
...
def CheckNumbersUsedAreAllInNumbersAllowed(NumbersAllowed, UserInputInRPN, MaxNumber):
    """
    Checkes they're all in Numbers allowed, but also that no number is used twice!
    """
    Temp = []
...
def ConvertToRPN(UserInput):
    Position = 0 # index at which we are looking in UserInput
    Precedence = {"+": 2, "-": 2, "*": 4, "/": 4} # dict of operators as key, precedence as value. larger means eval first
    Operators = [] # buffer of operators we have encountered
    Operand, Position = GetNumberFromUserInput(UserInput, Position) # reads the number at the start, returns it(int), new pos
    UserInputInRPN = [] # the stack in which we will build the RPN expr
    UserInputInRPN.append(str(Operand))
    Operators.append(UserInput[Position - 1])
    while Position < len(UserInput):
        Operand, Position = GetNumberFromUserInput(UserInput, Position) #reads next operAND
        UserInputInRPN.append(str(Operand)) #reads next operatOR
...
def GetNumberFromUserInput(UserInput, Position):
    ...
    while MoreDigits:
        if not(re.search("[0-9]", str(UserInput[Position])) is None):
            ...
            MoreDigits = False            
        Position += 1 # if this line was after Number +=... Position would be correct after call
        if Position == len(UserInput):
            MoreDigits = False
    if Number == "":
        return -1, Position
