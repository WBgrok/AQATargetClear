...
def UpdateTargets(Targets, TrainingGame, MaxTarget):
    for Count in range (0, len(Targets) - 1):
        Targets[Count] = Targets[Count + 1] # cycle the list forward 
    Targets.pop() # remove last element
    if TrainingGame:
        Targets.append(Targets[-1]) # fill with the current last element 
    else:
        Targets.append(GetTarget(MaxTarget)) # fill with new random targets
    return Targets
...
def EvaluateRPN(UserInputInRPN):
    ...
    while len(UserInputInRPN) > 0:
        ...
        Num1 = float(S[-1])
        S.pop()
        Result = 0.0 # initialises a float 0 
        if UserInputInRPN[0] == "+":
            Result = Num1 + Num2
        ...
        UserInputInRPN.pop(0)
        S.append(str(Result))
    # when we reach this, s[0] (top of stack) contains the result       
    if float(S[0]) - math.floor(float(S[0])) == 0.0:
        return math.floor(float(S[0])) # if the result is integer, return it as an integer 
    else:
        return -1 # if the results had decimal points, the expression was invalid. 
